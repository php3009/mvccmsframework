<script type="text/javascript">
	...
	var attBoxCnt = <?php echo $attributeListEditArr[0]; ?>;
	function generatevariants(cnt) {
		cnt = parseInt(cnt);
		selCnt = 0;
		var attrArr = new Array(); 
		for(var i=1; i<=cnt; i++) {
			var attr = document.getElementById('attribute_'+i);
			var attrSelIndex = attr.selectedIndex;
			var attrId = parseInt(attr.options[attrSelIndex].value);
			if(attrId == 0) {
				selCnt++;    	
			}
			else {
				attrArr.push(i + "_" + attrId);
			}
		}
		if(selCnt == cnt) {
			...
			var attributelistareaSpan = document.getElementById('attributelistarea'); 
			var newspan = document.createElement('span');
			newspan.setAttribute('id', 'attBox_' + attBoxCnt);
			var innerHTMLString = "<br /><br />";
			var nameStr = '';
			var j = 0;
			for(var j in attrArr) {
				attrArrTemp = attrArr[j].split('_'); 
				var attrName = document.getElementById('attributename_' + attrArrTemp[0]).innerHTML;
				var attr = document.getElementById('attribute_'+attrArrTemp[0]);
				var attrSelIndex = attr.selectedIndex;
				nameStr += attr.options[attrSelIndex].value + "_";
				var attrText = attr.options[attrSelIndex].text;
				//innerHTMLString += attrName + " : " + attrText + " // ";
				innerHTMLString += attrText + " >> ";
				j++;
			}
			nameStr = nameStr.substring(0, nameStr.length-1);
			var check = document.getElementById('vmin_'+nameStr);
			if(check == null) {
				innerHTMLString = innerHTMLString.substring(0, innerHTMLString.length-4);
				innerHTMLString += "&nbsp;&nbsp;&nbsp;Min&nbsp;:&nbsp;<input type='text' name='vmin_"+nameStr+"' id='vmin_"+nameStr+"' class='adminTableInputTextNoSKU'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Max&nbsp;:&nbsp;<input type='text' name='vmax_"+nameStr+"' id='vmax_"+nameStr+"' class='adminTableInputTextNoSKU'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In Stock&nbsp;:&nbsp;<input type='text' name='vstock_"+nameStr+"' id='vstock_"+nameStr+"' class='adminTableInputTextNoSKU'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;SKU&nbsp;:&nbsp;<input type='text' name='vsku_"+nameStr+"' id='vsku_"+nameStr+"' class='adminTableInputTextNoSKU'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Price&nbsp;:&nbsp;<input type='text' name='vprice_"+nameStr+"' id='vprice_"+nameStr+"' class='adminTableInputTextNoSKU'><a href=\'javascript:void(0);\' onclick=\"removegeneratedvariant('attBox_"+attBoxCnt+"');\" class=\'addnewimage1\'>Remove</a>";
				
				newspan.innerHTML = innerHTMLString;

				attributelistareaSpan.appendChild(newspan);
				
				attBoxCnt++;
			}
			else {
				alert("Product Variant is already generated.");
			}
		}
	}

	function removegeneratedvariant(obj) {
		var parentspan = document.getElementById('attributelistarea');
		oldspan = document.getElementById(obj);
		parentspan.removeChild(oldspan);
	}
...
<table border="0" width="100%">
	<tr>
		<td width="50%">
			<span class="application_title">Product Manager :</span><span class="application_job">[With Attributes - Edit]</span>
		</td>
		<td align="right">
		...
		<div class="TabbedPanelsContentGroup">
			<div class="TabbedPanelsContent"><br/>
				<table width="100%">
					...
					<tr>
						<td colspan="2">
							<input type="hidden" name="job" id="job" value="editsaveattributeproduct">
							<input type="hidden" name="pid" id="pid" value="<?php echo $id;?>">
							<input type="hidden" name="vendorId" id="vendorId" value="<?php echo $vendor_id;?>">
			...
			<div class="TabbedPanelsContent"><br/>
				<table width="100%">
					...
					<tr>
						...
						<td>
							<span id="attributelistarea" name="attributelistarea" class="attributelist"><?php
								$this->getAttributeList($vendor_id); 
								echo $attributeListEditArr[1];
							?></span>
						</td>
					</tr>
				</table>
